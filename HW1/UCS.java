...
public class UCS
    ...
    public String search(int W, int H, Point landingSite, int Z, int[][] terrainMap, List<Point> targetSites) {
        ...
        for (Point target : targetSites) {
            path.append(UCSSearch(W, H, landingSite, target, Z, terrainMap) + "\n");
        }
        //System.out.println("Time-" + (System.currentTimeMillis() - startTS));
        return path.toString().trim();
    }

    private StringBuilder UCSSearch(int W, int H, Point landingSite, Point target, int Z, int[][] terrainMap) {

        PriorityQueue<Node> priorityQueue = new PriorityQueue<>(1, Comparator.comparingInt(Node::getCost));

        Set<Node> visited = new HashSet<>();
        Node landingNode = new Node(landingSite, 0);
        priorityQueue.offer(landingNode);
        ...
        do {
            ...
            List<Node> neighbourList = getValidNeighboursOfNode(W, H, currentPoint.getPoint().getX(), currentPoint.getPoint().getY(), Z, terrainMap, target, "UCS");
            for (Node node : neighbourList) {
                node.setCost(currentPoint.getCost() + node.getCost());
